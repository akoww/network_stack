...
## Key Components
- **`ClientBase.h`**: Shared config (`host()`, `port()`, `getIoContext()`, `getSslContext()`). Defines `ClientSync` and `ClientAsync` interfaces.
...
- TLS connections reuse the lazily-created `getSslContext()` from the base class.
