...
- **`TcpSocket` / `TlsSocket`**: Concrete implementations inheriting `DualSocket`. `TlsSocket` requires explicit `asio/ssl/` includes.
- **`SocketBaseDetails.h`**: Internal helpers in `socket_detail` namespace (not public API).
