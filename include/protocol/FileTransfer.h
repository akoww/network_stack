...
public:
  ...
  virtual std::expected<std::vector<std::byte>, std::error_code> read(const std::filesystem::path& path) = 0;
  ...
  using ReadCallback = std::function<std::expected<std::size_t, std::error_code>(std::span<const std::byte>)>;
  ...
  using WriteCallback = std::function<std::expected<std::span<const std::byte>, std::error_code>()>;
  virtual std::expected<FileListData, std::error_code> write(const std::filesystem::path& remote_dst_path,
  ...
  virtual std::expected<bool, std::error_code> isDirectory(const std::filesystem::path& path) = 0;
