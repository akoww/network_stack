 ...
 * target location. It is protocol-agnostic: derive from this class and implement
 * ftpCd and ftpSelectDrive to hook into your transport (FTP, SFTP, etc.).
 *
 * Characteristics:
 * - Purely lexical: paths are normalized using std::filesystem::path::lexically_normal;
 *   no filesystem access is performed and symlinks are not resolved.
 * - Cross-platform: handles Unix-like roots ("/") and Windows drive roots ("C:").
 * - Minimal navigation: goes up to the common ancestor and then down to the target.
 *
...
public:
   ...
   *                  The path is lexically normalized and stored as the current location.
   */
  explicit SmartDirectoryNavigator(const std::filesystem::path& startPath);
  virtual ~SmartDirectoryNavigator() = default;

  /**
   * @brief Navigate from the current path to targetPath using incremental steps.
   *
   * Behavior:
   ...
   *   then the navigator descends from the root by issuing ftpCd for each component.
   * - Otherwise, computes the common prefix of current and target:
   *   - Calls ftpCd("..") once per level to go up from current to the common ancestor.
   *   - Calls ftpCd(component) for each remaining component to reach the target.
   *
   * The internal current path is updated to the normalized targetPath.
   *
   * @param targetPath Desired path to navigate to.
   */
  void changeDirectory(const std::filesystem::path& targetPath);

protected:
  /**
   * @brief Issue a single directory change on the underlying protocol.
   *
   * Implementations should perform a "cd" into dir or handle ".." to move up one level.
   * This is called with exactly one path component at a time or "..".
   *
   * @param dir Either a directory name (one path element) or ".." to go up.
   */
  virtual void ftpCd(const std::string& dir) = 0;

  /**
   * @brief Switch the session to a different drive/root when needed (Windows).
   *
   * Called when changeDirectory detects that target.root_name() differs from the
   * current root and target.root_name() is non-empty.
   *
   * @param drive The drive/root identifier, e.g., "D:".
   */
  virtual void ftpSelectDrive(const std::string& drive) = 0;
...
private:
   ...
   *
   * Used after a root/drive change. Iterates target.relative_path() and calls ftpCd
   * for each element in order.
   *
   * @param target Absolute target path.
   */
  void descendFromRoot(const std::filesystem::path& target);
};
