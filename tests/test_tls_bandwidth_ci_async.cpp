...
TEST_F(AsyncClientServerFixture, AsyncTlsCiBandwidth)
{
  ...
  constexpr std::size_t chunk_size = 128 * 1024;         // 64KB chunks
  constexpr std::size_t total_bytes = 128 * chunk_size;  // 10MB
  ...
  while (bytes_sent < total_bytes)
  {
    ...
    {
      auto to_read = static_cast<std::size_t>(std::min(std::size_t{4096 * 16}, chunk - bytes_read));
      std::array<std::byte, 4096 * 16> bufs{};
