...
private:
  ...
  static constexpr const char* kTcpsvdArgs = "/usr/bin/busybox tcpsvd -vE 0.0.0.0 2121";
...
{
  serverArgs_ = std::string(kTcpsvdArgs) + " " + std::string(FtpServerFixture::kFtpdProgram) + " ftpd -A -w /tmp/ftpd";
}
...
bool FtpServerFixture::isRunning() const
{
  return running_;
}
...
bool FtpServerFixture::startServerProcess()
{
  ...
  cmd << kTcpsvdArgs << " " << FtpServerFixture::kFtpdProgram << " ftpd -A -w /tmp/ftpd &";
  ...
  std::string pgrepCmd = "pgrep -f \"" + std::string(kTcpsvdArgs) + "\" | head -1";
