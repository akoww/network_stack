...
TEST_F(IoContextFixture, MultipleClientsConcurrent) {
  ...
  client3.join();
  std::this_thread::sleep_for(std::chrono::milliseconds(50));
  EXPECT_TRUE(server.getSockets().size() >= 1) << "Expected at least 1 connection";
  server.stop();
  server_thread.join();
}
...
TEST_F(SyncClientServerFixture, EchoServerMultipleMessages) {
  ...
  public:
    ...
    void handle_client(std::unique_ptr<TcpSocket> sock) override {
      ...
      while (true) {
        auto recv_result = sock->receive(std::span(buffer));
        if (!recv_result || *recv_result == 0) break;
        auto send_result = sock->send(std::span(buffer).first(*recv_result));
  ...
  if (connect_result) {
    ...
    for (const auto& msg : messages) {
      auto send_result = _client_socket->send(to_bytes(msg));
      EXPECT_TRUE(send_result);
      std::array<std::byte, 1024> buffer{};
      auto recv_result = _client_socket->receive(std::span(buffer));
...
TEST_F(SyncClientServerFixture, EchoServerConcurrentClients) {
  ...
  for (int i = 0; i < 3; i++) {
    ...
    if (connect_result) sockets.push_back(std::move(*connect_result));
  }
  std::this_thread::sleep_for(std::chrono::milliseconds(50));
  auto send_recv = [&](TcpSocket& socket, const std::string& msg) {
    auto send_result = socket.send(to_bytes(msg));
    EXPECT_TRUE(send_result);
    std::array<std::byte, 1024> buffer{};
    auto recv_result = socket.receive(std::span(buffer));
    EXPECT_TRUE(recv_result);
    if (recv_result) {
      auto response = to_string_view(buffer, *recv_result);
      EXPECT_EQ(msg, response);
...
TEST_F(SyncClientServerFixture, ServerRestart) {
  ...
  std::this_thread::sleep_for(std::chrono::milliseconds(50));
  auto connect_result = _client.connect({});
  if (connect_result) _client_socket = std::move(*connect_result);
  std::this_thread::sleep_for(std::chrono::milliseconds(50));
  server.stop();
  server_thread_.join();
  std::this_thread::sleep_for(std::chrono::milliseconds(50));
...
TEST_F(SyncClientServerFixture, SpecialCharacters) {
  ...
  if (connect_result) {
    ...
    auto send_result = _client_socket->send(to_bytes(special));
    EXPECT_TRUE(send_result);
    std::array<std::byte, 1024> buffer{};
    auto recv_result = _client_socket->receive(std::span(buffer));
    EXPECT_TRUE(recv_result);
...
TEST_F(SyncClientServerFixture, BinaryData) {
  ...
  if (connect_result) {
    ...
    for (size_t i = 0; i < 256; i++) binary_data[i] = static_cast<std::byte>(i);
    auto send_result = _client_socket->send(std::span(binary_data));
    EXPECT_TRUE(send_result);
    std::array<std::byte, 512> buffer{};
    auto recv_result = _client_socket->receive(std::span(buffer));
    EXPECT_TRUE(recv_result);
    if (recv_result && size_t(*recv_result) == 256) {
      for (size_t i = 0; i < 256; i++) EXPECT_EQ(binary_data[i], buffer[i]);
    }
  }
  server.stop();
  server_thread_.join();
