...
TEST_F(AsyncClientServerFixture, ZeroSizeWrite)
{
  ...
  asio::co_spawn(
    _io_ctx,
    [&client_socket, empty_span, promise = std::move(send_promise)]() mutable -> asio::awaitable<void>
    {
...
TEST_F(AsyncClientServerFixture, ZeroSizeRead)
{
  ...
  asio::co_spawn(
    _io_ctx,
    [&client_socket, &empty_span, promise = std::move(recv_promise)]() mutable -> asio::awaitable<void>
    {
...
TEST_F(AsyncClientServerFixture, ServerAbruptShutdownDuringRead)
{
  ...
  asio::co_spawn(
    _io_ctx,
    [&client_socket, promise = std::move(recv_promise)]() mutable -> asio::awaitable<void>
    {
...
TEST_F(AsyncClientServerFixture, FragmentedWriteRead)
{
  ...
  for (char c : message)
  {
    ...
    asio::co_spawn(
      _io_ctx,
      [&client_socket, send_buffer, promise = std::move(send_promise)]() mutable -> asio::awaitable<void>
      {
...
TEST_F(AsyncClientServerFixture, SpecialCharactersWithNullBytes)
{
  ...
  asio::co_spawn(
    _io_ctx,
    [&client_socket, special, promise = std::move(send_promise)]() mutable -> asio::awaitable<void>
    {
  ...
  if (recv_result)
  {
    auto response = std::string(reinterpret_cast<const char*>(buffer.data()), *recv_result);
...
TEST_F(AsyncClientServerFixture, BinaryDataAllBytes)
{
  ...
  asio::co_spawn(
    _io_ctx,
    [&client_socket, &binary_data, promise = std::move(send_promise)]() mutable -> asio::awaitable<void>
    {
      auto result = co_await client_socket->asyncWriteAll(std::span(binary_data));
      promise.set_value(std::move(result));
    },
    asio::detached);
