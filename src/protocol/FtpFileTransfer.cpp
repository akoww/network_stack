...
DefaultFtpNavigator::DefaultFtpNavigator(FtpFileTransfer* parent) : SmartDirectoryNavigator("/"), _parent(parent)
{
}

std::expected<void, std::error_code> DefaultFtpNavigator::ftpCd(const std::string& dir)
{
  spdlog::info("cwd {}", dir);
  auto cmd_result = _parent->sendAndReceiveResponse(std::format("CWD {}", dir));
  if (!cmd_result)
  {
    spdlog::warn("Failed to change directory to '{}': {}", dir, cmd_result.error().message());
...
std::expected<void, std::error_code> DefaultFtpNavigator::ftpSelectDrive(const std::string& drive)
{
  auto cmd_result = _parent->sendAndReceiveResponse(std::format("CWD {}", drive));
  if (!cmd_result)
  {
    spdlog::warn("Failed to change directory to '{}': {}", drive, cmd_result.error().message());
    return std::unexpected(cmd_result.error());
  }
  if (cmd_result->code != 250)
  {
...
auto as_byte_span(const Container& c)
{
  ...
  return std::span<const std::byte>(reinterpret_cast<const std::byte*>(std::ranges::data(c)), std::ranges::size(c));
}
...
std::expected<void, std::error_code> FtpFileTransfer::connect(const ConnectOptions& opts)
{
  ...
  auto recv_result = _socket->readUntil(std::span(buffer), "\r\n");
...
bool isCompleteFtpResponse(std::ranges::input_range auto&& lines)
{
  ...
  if (first.size() < 4 || !std::isdigit(first[0]) || !std::isdigit(first[1]) || !std::isdigit(first[2]))
  {
    return false;
  }
...
template <typename C>
concept ContiguousByteContainer = requires(C c)
{
  std::data(c);
  std::size(c);
}
&&std::is_trivial_v<std::remove_pointer_t<decltype(std::data(std::declval<C&>()))>> &&
  sizeof(std::remove_pointer_t<decltype(std::data(std::declval<C&>()))>) == 1;

template <ContiguousByteContainer C>
std::string_view to_string_view(const C& container)
{
  return std::string_view(reinterpret_cast<const char*>(std::data(container)), std::size(container));
}
...
std::expected<void, std::error_code> FtpFileTransfer::sendCommand(std::string_view cmd)
{
  return sendCommand(*_socket, cmd, _options.timeout);
}

std::expected<FtpFileTransfer::Answer, std::error_code> FtpFileTransfer::receiveResponse()
{
  return receiveResponse(*_socket, _options.timeout);
}

std::expected<FtpFileTransfer::Answer, std::error_code> FtpFileTransfer::sendAndReceiveResponse(std::string_view cmd)
{
...
std::expected<void, std::error_code> FtpFileTransfer::sendCommand(SyncSocket& sock,
                                                                  std::string_view cmd,
                                                                  std::chrono::milliseconds timeout)
{
  const std::string command = std::format("{}\r\n", cmd);
  if (auto r = sock.writeAll(as_byte_span(command), timeout); !r)
  {
...
std::expected<FtpFileTransfer::Answer, std::error_code> FtpFileTransfer::receiveResponse(
  SyncSocket& sock, std::chrono::milliseconds timeout)
{
  std::string buf;
  ...
  while (true)
  {
    auto r = sock.readUntil(std::span(tmp), "\r\n", timeout);
    if (!r)
    {
...
std::expected<std::vector<std::byte>, std::error_code> FtpFileTransfer::receiveRawResponse(
  SyncSocket& sock, std::chrono::milliseconds timeout)
{
  ...
  while (true)
  {
    auto r = sock.readSome(std::span(tmp), timeout);
    if (!r)
    {
...
std::expected<FtpFileTransfer::Answer, std::error_code> FtpFileTransfer::sendAndReceiveResponse(
  SyncSocket& sock, std::string_view cmd, std::chrono::milliseconds timeout)
{
...
std::chrono::system_clock::time_point parseFtpTimestamp(const std::string& month,
                                                        const std::string& day,
                                                        const std::string& time_or_year)
{
  // Map month names to month numbers (0-11)
  static const std::map<std::string, int> monthMap = {{"Jan", 0}, {"Feb", 1}, {"Mar", 2},  {"Apr", 3},
...
std::optional<FileListData> parseFtpLine(std::string_view line)
{
  ...
  if (!(iss >> perms >> link_count >> owner >> group >> size >> month >> day >> time_or_year))
  {
  ...
  while (!file.file_name.empty() && (std::isspace(static_cast<unsigned char>(file.file_name.back())) != 0))
  {
    file.file_name.pop_back();
  }
  // Trim leading whitespace (getline might pick it up if there were double
  // spaces)
  while (!file.file_name.empty() && (std::isspace(static_cast<unsigned char>(file.file_name.front())) != 0))
  {
...
std::expected<std::vector<FileListData>, std::error_code> FtpFileTransfer::list(const std::filesystem::path& path)
{
  ...
  {
    spdlog::error("Failed to open data connection for LIST: {}", data_socket_result.error().message());
...
std::expected<void, std::error_code> FtpFileTransfer::createDir(const std::filesystem::path& path)
{
  ...
  {
    // protocol error
    spdlog::error("cant check if directory exists: {}", path.parent_path().string());
  ...
  if (auto change_result = _navigator.changeDirectory(path.parent_path()); !change_result)
  {
    spdlog::error("cant change directory to: {}", path.parent_path().string());
    return std::unexpected(change_result.error());
  }

  auto cmd_result = sendAndReceiveResponse(std::format("MKD {}", get_last_component(path).string()));
  if (!cmd_result)
  {
    spdlog::error("MKD command failed for '{}': {}", path.string(), cmd_result.error().message());
    return std::unexpected(cmd_result.error());
  }
...
std::expected<bool, std::error_code> FtpFileTransfer::existsMlst(std::string_view name)
{
...
std::expected<bool, std::error_code> FtpFileTransfer::existsSize(std::string_view name)
{
  ...
  if (cmd_result->code == 550 || cmd_result->code == 500)
  {
    return false;
  }
...
std::expected<bool, std::error_code> FtpFileTransfer::exists(const std::filesystem::path& remote_path)
{
  ...
  if (auto change_result = _navigator.changeDirectory(remote_path.parent_path()); !change_result)
  {
    spdlog::error("cant change directory to: {}", remote_path.parent_path().string());
    return std::unexpected(change_result.error());
  }
...
std::expected<void, std::error_code> FtpFileTransfer::remove(const std::filesystem::path& remote_path)
{
  ...
  if (!is_dir_result)
  {
    spdlog::error("Failed to determine if '{}' is directory: {}", remote_path.string(),
                  is_dir_result.error().message());
  ...
  auto cmd_result = sendAndReceiveResponse(std::format("{} {}", cmd, remote_path.filename().string()));
  if (!cmd_result)
  {
    spdlog::error("{} command failed for '{}': {}", cmd, remote_path.string(), cmd_result.error().message());
    return std::unexpected(cmd_result.error());
  }
  if (cmd_result->code != 250)
  {
...
std::expected<std::vector<std::byte>, std::error_code> FtpFileTransfer::read(const std::filesystem::path& path)
{
  ...
  if (!data_socket_result)
  {
    spdlog::error("Failed to open data connection for RETR: {}", data_socket_result.error().message());
    return std::unexpected(data_socket_result.error());
  }
  auto data_socket = std::move(*data_socket_result);

  auto cmd_result = sendAndReceiveResponse(std::format("RETR {}", path.filename().string()));
  if (!cmd_result)
  {
  ...
  {
    spdlog::error("RETR read data failed: {}", receive_result.error().message());
  ...
  if (!response)
  {
    spdlog::error("Failed to receive RETR completion response: {}", response.error().message());
                                                                  ...
                                                                  ReadCallback callback)
{
  ...
  while (true)
  {
    auto read_result = data_socket->readSome(std::span(buffer), getDataTimeout());
    if (!read_result)
    {
    ...
    if (!callback_result)
    {
      spdlog::error("Callback returned error: {}", callback_result.error().message());
                                                                    ...
                                                                    std::span<const std::byte> data)
{
  ...
  // naviagte to parent path
  if (auto change_result = _navigator.changeDirectory(remote_dst_path.parent_path()); !change_result)
  {
    spdlog::error("cant change directory to: {}", remote_dst_path.parent_path().string());
    return std::unexpected(change_result.error());
  }
  ...
  if (!data_socket_result)
  {
    spdlog::error("Failed to open data connection for STOR: {}", data_socket_result.error().message());
    return std::unexpected(data_socket_result.error());
  }
  auto data_socket = std::move(*data_socket_result);

  auto cmd_result = sendAndReceiveResponse(std::format("STOR {}", remote_dst_path.filename().string()));
  if (!cmd_result)
  {
  ...
  auto send_result = data_socket->writeAll(buffer, getDataTimeout());
  if (!send_result)
  {
    spdlog::error("Failed to send file data: {}", send_result.error().message());
    return std::unexpected(send_result.error());
  }
  ...
  if (!response)
  {
    spdlog::error("Failed to receive STOR completion response: {}", response.error().message());
    return std::unexpected(response.error());
  }
                                                                    ...
                                                                    WriteCallback next)
{
  ...
  while (true)
  {
    ...
    if (!chunk_result)
    {
      spdlog::error("Callback returned error: {}", chunk_result.error().message());
    ...
    auto send_result = data_socket->writeAll(chunk, getDataTimeout());
    if (!send_result)
    {
      spdlog::error("Failed to send file data: {}", send_result.error().message());
      return std::unexpected(send_result.error());
...
std::expected<bool, std::error_code> FtpFileTransfer::isDirectoryMlst(std::string_view name)
{
  if (!_capabilities.mlst)
  {
    return false;
  ...
  if (cmd_result->code == 250)
  {
    ...
    std::string type_field = cmd_result->full_msg.substr(start, end == std::string::npos ? end : end - start);
...
std::expected<bool, std::error_code> FtpFileTransfer::isDirectoryCwd(std::string_view name)
{
  ...
  if (cmd_result->code == 550)
  {
    if (_capabilities.size)
    {
      ...
      if (!size_result)
      {
        spdlog::error("SIZE check failed for '{}': {}", name, size_result.error().message());
...
std::optional<asio::ip::tcp::endpoint> parsePasvResponse(std::string_view response)
{
  std::error_code ec;
  ...
  while (pos < params.size() && count < 6)
  {
    ...
    size_t len = (comma == std::string_view::npos) ? (params.size() - pos) : (comma - pos);
    ...
    if (ec)
    {
      spdlog::error("PASV parameter parse error at '{}' : {}", part, ec.message());
      return std::nullopt;
    }
  ...
  auto port = static_cast<uint16_t>((parts[4] << 8) | parts[5]);
...
std::expected<std::unique_ptr<SyncSocket>, std::error_code> FtpFileTransfer::openDataConnection()
{
  ...
  auto data_socket = std::make_unique<ClientSync>(endpoint->address().to_string(), endpoint->port(), _io_context);
  auto connect_result = data_socket->connect({getDataTimeout()});
  if (!connect_result)
  {
    spdlog::error("Failed to connect data socket: {}", connect_result.error().message());
...
void FtpFileTransfer::parseFeatures(std::string_view feat_response)
{
  ...
  for (const auto& line : lines)
  {
    ...
    if (line[0] == ' ' && line.size() > 1)
    {
      ...
      while (end_pos > 0 &&
             (feature[end_pos - 1] == ' ' || feature[end_pos - 1] == '\r' || feature[end_pos - 1] == '\n'))
      {
        --end_pos;
      }
